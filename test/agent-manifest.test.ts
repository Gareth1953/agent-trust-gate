...
test("agent manifest exposes stable discovery, capabilities, and disabled commerce", () => {
  ...
  for (const capability of ["pre_action_trust_decision", "approval_pack_generation", "human_review_evidence", "evidence_bundle_export", "gateway_usage_metering", "local_client_allowances", "local_admin_summary", "agent_entitlement_status", "commercial_readiness_snapshot", "hosted_deployment_readiness", "production_security_readiness", "local_rate_limit_and_abuse_signals", "local_monitoring_health_signals", "incident_response_and_recovery_readiness", "customer_tenant_readiness", "billing_payment_readiness", "machine_purchase_policy_readiness", "public_developer_launch_readiness", "global_marketing_distribution_readiness"]) {
  ...
  assert.equal(manifest.tools.some((tool) => (
    tool.name === "atg_get_hosted_readiness" &&
    tool.path === "/v1/hosted-readiness" &&
    tool.executes_actions === false
  )), true);
  assert.equal(manifest.tools.some((tool) => (
    tool.name === "atg_get_security_readiness" &&
    tool.path === "/v1/security-readiness" &&
    tool.executes_actions === false
  )), true);
  assert.equal(manifest.tools.some((tool) => (
  ...
  assert.equal(manifest.tools.some((tool) => (
    tool.name === "atg_get_incident_response_readiness" &&
    tool.path === "/v1/incident-response-readiness" &&
    tool.executes_actions === false
  )), true);
  assert.equal(manifest.tools.some((tool) => (
    tool.name === "atg_get_customer_tenant_readiness" &&
    tool.path === "/v1/customer-tenant-readiness" &&
    tool.executes_actions === false
  )), true);
  assert.equal(manifest.tools.some((tool) => (
    tool.name === "atg_get_billing_payment_readiness" &&
    tool.path === "/v1/billing-payment-readiness" &&
    tool.executes_actions === false
  )), true);
  assert.ok(manifest.tools.some((tool)=>tool.name==="atg_get_machine_purchase_policy_readiness"&&tool.executes_actions===false));
  assert.ok(manifest.tools.some((tool)=>tool.name==="atg_get_launch_readiness"&&tool.path==="/v1/launch-readiness"&&tool.executes_actions===false));
  assert.ok(manifest.tools.some((tool)=>tool.name==="atg_get_global_marketing_readiness"&&tool.path==="/v1/global-marketing-readiness"&&tool.executes_actions===false));
  assert.equal(manifest.tools.some((tool) => (
