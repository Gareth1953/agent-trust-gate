...
test("hosted readiness report is versioned and keeps deployment disabled", () => {
  ...
  assert.equal(report.overall.hosted_readiness_percent, 42);
  ...
  assert.equal(report.checks.some((check) => check.id === "local_monitoring_health_available"), true);
  assert.equal(report.checks.some((check) => check.id === "local_incident_response_readiness_available"), true);
  assert.equal(report.checks.some((check) => check.id === "local_customer_tenant_readiness_available"), true);
  assert.equal(report.checks.some((check) => check.id === "local_billing_payment_readiness_available"), true);
  assert.equal(report.checks.some((check) => check.id === "local_public_launch_readiness_available"), true);
});
...
test("hosted readiness contains required security and missing-capability checks", () => {
  ...
  for (const id of [
    ...
    "production_monitoring_local_only",
