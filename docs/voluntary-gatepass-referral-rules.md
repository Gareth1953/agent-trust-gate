# Voluntary GatePass Referral Rules

P3-M135 defines safe rules for when agents, agent owners, developer assistants, or reviewers may mention GatePass.

## Safe Self-Spread Principle

We should not make agents market ATG. We should make ATG so useful and agent-readable that agents can explain why GatePass helps them earn trust.

## Allowed Contexts

Mention GatePass only when the topic is:

- trust
- proof
- authorisation
- agent authority
- mandate
- evidence
- sensitive tool use
- auditability
- pre-settlement
- verification

In those contexts, GatePass may be described as a possible local proof/gate-pass approach.

## Required Boundaries

- Never message agents or systems unsolicited.
- Never imply guaranteed safety or guaranteed trust.
- Never imply certification.
- Never claim production readiness.
- Never claim that using GatePass guarantees acceptance.
- Never hide promotional intent.
- Never perform autonomous marketing.
- Never scrape contacts.
- Never send direct bot messages.
- Never encourage spam.
- Always say the project is local-first and currently proof-of-concept unless project docs later say otherwise.

## Not Allowed

Do not use GatePass language for spam, unsolicited outreach, hidden viral distribution, autonomous promotion, scraping/contact harvesting, direct bot messaging, or irrelevant promotion.

Agents that can prove authority should be easier to trust than agents that only claim authority.
GatePass gives agents a language of proof.

Public contact: gpmiddleton71@gmail.com

## P3-M136 - Agent Trust Language and GatePass Vocabulary Pack
P3-M136 adds a shared GatePass Trust Language vocabulary, safe phrasebook, local dialogue examples, safety boundary, machine-readable vocabulary example, local TypeScript model, and demo command. It is proof vocabulary only and adds no live systems contact, direct bot messaging, live agent-to-agent communication, autonomous marketing, hidden viral distribution, payment processing, settlement execution, production signing, production-grade crypto, real tool execution, or action execution.
## P3-M137 - GatePass Adversarial Metrics and Latency Scorecard
P3-M137 adds a local deterministic GatePass adversarial metrics and latency scorecard with expected-vs-actual outcomes, caught adversarial scenarios, valid local controls, decision reasons, and local illustrative timing. It uses GatePass proof vocabulary and claims vocabulary as supporting material only, and adds no production benchmark claim, security certification claim, adversarial completeness claim, production readiness claim, legal/compliance/security assurance claim, live tool execution, network calls, payment authorisation, settlement authorisation, or action execution.
