...
Authentication can establish which account or fixture identity participated. It does not by itself answer: Who is accountable for this agent? What was delegated? Does configured evidence show current organisational authority for this exact action? Does configured evidence show that the organisation’s independent-verification step was completed? Did two independent people approve the same canonical action? Is the approval fresh and unused? Does the action presented now match exactly?

## What ATG tests

ATG locally evaluates one synthetic control chain: agent identity, accountable principal, scoped delegation, Agent Standing and an exact supplier master-data proposal. Configured evidence shows that the organisation’s independent-verification step was completed; ATG does not determine whether the bank details are correct. ATG then evaluates configured evidence of current organisational authority for this exact action, separation of duties, dual approval, freshness, replay state and the exact-action digest. The output is either a one-use GatePass for that exact proposal or a refusal with a stable reason code and comparison evidence.
...
The fictional Procurement Change Agent 12, accountable to a fictional Procurement Operations Department, proposes changing `SUP-10482`, Northbridge Office Systems Ltd, from an account ending `1846` to one ending `7319`. The agent’s identity, principal, supplier-maintenance delegation and Agent Standing verify within the local fixture. Configured evidence shows that the organisation’s independent-verification step was completed; ATG does not determine whether the bank details are correct. Configured evidence shows that two different active human fixture identities have current organisational authority for this exact action and approve the same canonical digest. The approval remains inside 30 minutes and the nonce is unused. ATG does not independently establish legal authority.
...
2. **Missing independent verification — `INDEPENDENT_VERIFICATION_MISSING`.** A supplier request exists, but configured evidence does not show that the organisation’s independent-verification step was completed. ATG does not determine whether the bank details are correct. The policy refuses before GatePass issuance. No external action occurs.
