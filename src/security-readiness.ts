...
function securityChecks(): SecurityReadinessCheck[] {
  return [
    ...
    check("customer_tenant_readiness_local_only", "Customer and tenant readiness is local only", "partial", "warning", ["A non-personal local account, tenant, and client ownership planning model exists."], "Implement production authentication, authorization, tenant isolation, secure storage, privacy, deletion, and support controls."),
    ...
    check("public_docs_safety_boundaries_available", "Public documentation safety boundaries available", "partial", "info", ["Local developer docs explicitly avoid production, legal, compliance, payment, and execution overclaims."], "Complete independent security and legal review before publishing or hosting."),
  ];
}
