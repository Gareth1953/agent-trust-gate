...
import { createGlobalMarketingReadinessReport, formatGlobalMarketingReadinessForConsole, writeGlobalMarketingReadinessReport } from "./global-marketing-readiness.js";
...
export function runCli(args: string[]): number {
  ...
  if (args.includes("--global-marketing-readiness")) return runGlobalMarketingReadinessMode(args,jsonMode);
...
function runGlobalMarketingReadinessMode(args:string[],json:boolean):number{let output:string|undefined;for(let i=0;i<args.length;i++){const a=args[i];if(a==="--global-marketing-readiness"||a==="--json")continue;if(a==="--output"){const v=args[++i];if(!v||v.startsWith("--")){printError("INVALID_GLOBAL_MARKETING_READINESS_ARGUMENTS","Missing output path after --output.",json);return 1;}output=v;continue;}printError("INVALID_GLOBAL_MARKETING_READINESS_ARGUMENTS",`--global-marketing-readiness cannot be combined with ${a}.`,json);return 1;}try{const r=createGlobalMarketingReadinessReport(),p=output?writeGlobalMarketingReadinessReport(output,r):undefined;console.log(json?JSON.stringify(r,null,2):formatGlobalMarketingReadinessForConsole(r));if(!json&&p)console.log(`\nSaved global marketing readiness report to ${p}`);return 0;}catch(e){printError("GLOBAL_MARKETING_READINESS_ERROR",`Unable to create global marketing readiness report: ${errorMessage(e)}`,json);return 1;}}
