...
function renderReviewerKit(report: GatePassReviewerKitReport, summaryOnly: boolean): string {
  ...
  const lines = [
    ...
    "runs: GatePass round trip, adversarial scorecard, developer wrapper, local framework-style integration summary, and optional embedded-commerce summary",
    "",
    ...
    `- command: ${report.commerceSummary.command}`,
    `- scenarios: ${report.commerceSummary.scenarioCount}`,
    `- allowed: ${report.commerceSummary.allowedCount}`,
    `- refused: ${report.commerceSummary.refusedCount}`,
    "- local synthetic pre-checkout basket verification only; no checkout or payment authorisation",
    "",
    "safety boundary:",
    `- ${getReviewerKitSafetyBoundary()}`,
