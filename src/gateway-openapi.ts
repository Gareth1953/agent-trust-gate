  ...
  return {
    ...
    paths: {
      ...
      "/v1/billing-payment-readiness": {
        get: {
          tags: ["Gateway"],
          summary: "Read local billing and payment readiness",
          description: "Returns local planning metadata only. It does not bill customers, collect payment details, process payments, create real invoices, enable automatic purchase, or execute actions.",
          parameters: [{ $ref: "#/components/parameters/ClientIdHeader" }],
          responses: {
            "200": response("Local billing and payment readiness report.", "BillingPaymentReadinessResponse"),
            "405": errors["405"],
            "500": errors["500"],
          },
        },
      },
      "/v1/machine-purchase-policy-readiness": { get: { tags:["Gateway"], summary:"Read local machine purchase policy readiness", description:"Planning only; no automatic purchase, billing, payment, or action execution.", parameters:[{$ref:"#/components/parameters/ClientIdHeader"}], responses:{"200":response("Local machine purchase policy readiness.","MachinePurchasePolicyReadinessResponse"),"405":errors["405"],"500":errors["500"]} } },
      "/v1/launch-readiness": { get: { tags:["Gateway"], summary:"Read local public developer and launch readiness", description:"Planning only; does not publish, deploy, collect signups, process payments, or enable automatic purchase.", parameters:[{$ref:"#/components/parameters/ClientIdHeader"}], responses:{"200":response("Local public developer documentation and launch readiness.","LaunchReadinessResponse"),"405":errors["405"],"500":errors["500"]} } },
      "/v1/global-marketing-readiness": { get: { tags:["Gateway"], summary:"Read local global marketing and distribution readiness", description:"Planning only; does not publish, deploy, send outreach, run ads, track users, collect signups, process payments, or enable automatic purchase.", parameters:[{$ref:"#/components/parameters/ClientIdHeader"}], responses:{"200":response("Local global marketing and distribution readiness.","GlobalMarketingReadinessResponse"),"405":errors["405"],"500":errors["500"]} } },
      "/v1/decision": {
        post: {
          tags: ["Trust"],
...
function createSchemas(): Record<string, unknown> {
  ...
  return {
    ...
    EntitlementResponse: {
      type: "object",
      required: ["contract_version", "entitlement_version", "request_id", "client_id", "tenant_id", "account_id", "plan_code", "payment_required", "local_only", "billing_enabled", "payment_processing_enabled", "automatic_purchase_enabled", "entitlement_status", "usage", "upgrade", "safety_statement"],
      properties: {
        contract_version: contractVersion,
        ...
        plan_code: { type: "null", description: "Reserved for future reviewed plan mapping; no active commercial tariff is selected." },
    ...
    DocumentationSection: { type:"object", required:["id","title","status","path","purpose","gaps"], properties:{id:{type:"string"},title:{type:"string"},status:{type:"string",enum:["complete","partial","missing","future"]},path:{type:"string"},purpose:{type:"string"},gaps:{type:"array",items:{type:"string"}}} },
    DeveloperAssets: { type:"object", description:"Local developer assets and disabled public distribution surfaces." },
    LaunchCheck: { type:"object", required:["id","label","status","severity","evidence","recommendation"], properties:{id:{type:"string"},label:{type:"string"},status:{type:"string"},severity:{type:"string"},evidence:{type:"array",items:{type:"string"}},recommendation:{type:"string"}} },
    LaunchReadinessResponse: { type:"object", required:["contract_version","launch_readiness_version","request_id","local_only","public_launch_enabled","public_docs_ready","hosted_service_enabled","package_published","payments_enabled","automatic_purchase_enabled","overall","documentation_sections","developer_assets","launch_checks","required_before_public_launch","recommended_launch_controls","safety_statement"], properties:{contract_version:contractVersion,launch_readiness_version:{type:"string",const:"atg.launch-readiness.v1"},request_id:{type:"string"},local_only:{type:"boolean",const:true},public_launch_enabled:{type:"boolean",const:false},public_docs_ready:{type:"boolean",const:false},hosted_service_enabled:{type:"boolean",const:false},package_published:{type:"boolean",const:false},payments_enabled:{type:"boolean",const:false},automatic_purchase_enabled:{type:"boolean",const:false},overall:{type:"object"},documentation_sections:{type:"array",items:{$ref:"#/components/schemas/DocumentationSection"}},developer_assets:{$ref:"#/components/schemas/DeveloperAssets"},launch_checks:{type:"array",items:{$ref:"#/components/schemas/LaunchCheck"}},required_before_public_launch:{type:"array",items:{type:"string"}},recommended_launch_controls:{type:"array",items:{type:"string"}},safety_statement:{type:"string"}} },
    DistributionChannel: { type:"object", required:["id","label","status","readiness_percent","purpose","current_assets","gaps","launch_risk","next_step"], properties:{id:{type:"string"},label:{type:"string"},status:{type:"string",enum:["not_started","partial","future","blocked"]},readiness_percent:{type:"integer",minimum:0,maximum:100},purpose:{type:"string"},current_assets:{type:"array",items:{type:"string"}},gaps:{type:"array",items:{type:"string"}},launch_risk:{type:"string"},next_step:{type:"string"}} },
    MarketingReadinessCheck: { type:"object", required:["id","label","status","severity","evidence","recommendation"], properties:{id:{type:"string"},label:{type:"string"},status:{type:"string",enum:["pass","partial","fail","not_started","future"]},severity:{type:"string",enum:["info","warning","critical"]},evidence:{type:"array",items:{type:"string"}},recommendation:{type:"string"}} },
    PositioningModel: { type:"object", required:["category","primary_audience","core_message","differentiators","overclaim_boundaries"], properties:{category:{type:"string",const:"pre-action trust gateway for AI agents"},primary_audience:{type:"array",items:{type:"string"}},core_message:{type:"string"},differentiators:{type:"array",items:{type:"string"}},overclaim_boundaries:{type:"array",items:{type:"string"}}} },
    DeveloperAdoptionAssets: { type:"object", description:"Local developer assets and disabled public distribution surfaces." },
    GlobalMarketingReadinessResponse: { type:"object", required:["contract_version","global_marketing_readiness_version","request_id","local_only","global_marketing_enabled","automated_outreach_enabled","public_distribution_enabled","analytics_tracking_enabled","paid_ads_enabled","signup_capture_enabled","outbound_agent_contact_enabled","automated_agent_outreach_enabled","external_agent_scanning_enabled","human_approval_required_before_contact","overall","positioning_model","agent_to_agent_discovery_readiness","distribution_channels","developer_adoption_assets","launch_message_assets","checks","required_before_global_distribution","recommended_distribution_controls","safety_statement"], properties:{contract_version:contractVersion,global_marketing_readiness_version:{type:"string",const:"atg.global-marketing-readiness.v1"},request_id:{type:"string"},local_only:{type:"boolean",const:true},global_marketing_enabled:{type:"boolean",const:false},automated_outreach_enabled:{type:"boolean",const:false},public_distribution_enabled:{type:"boolean",const:false},analytics_tracking_enabled:{type:"boolean",const:false},paid_ads_enabled:{type:"boolean",const:false},signup_capture_enabled:{type:"boolean",const:false},outbound_agent_contact_enabled:{type:"boolean",const:false},automated_agent_outreach_enabled:{type:"boolean",const:false},external_agent_scanning_enabled:{type:"boolean",const:false},human_approval_required_before_contact:{type:"boolean",const:true},overall:{type:"object"},positioning_model:{$ref:"#/components/schemas/PositioningModel"},agent_to_agent_discovery_readiness:{type:"object"},distribution_channels:{type:"array",items:{$ref:"#/components/schemas/DistributionChannel"}},developer_adoption_assets:{$ref:"#/components/schemas/DeveloperAdoptionAssets"},launch_message_assets:{type:"array",items:{type:"object"}},checks:{type:"array",items:{$ref:"#/components/schemas/MarketingReadinessCheck"}},required_before_global_distribution:{type:"array",items:{type:"string"}},recommended_distribution_controls:{type:"array",items:{type:"string"}},safety_statement:{type:"string"}} },
    MachinePurchasePolicyReadinessCheck: { type:"object", required:["id","label","status","severity","evidence","recommendation"], properties:{id:{type:"string"},label:{type:"string"},status:{type:"string"},severity:{type:"string"},evidence:{type:"array",items:{type:"string"}},recommendation:{type:"string"}} },
    PolicyModel: { type:"object", description:"Deny-by-default local purchase policy signals." },
    SpendingControlModel: { type:"object", required:["default_policy","automatic_purchase_limit"], properties:{default_policy:{type:"string",const:"deny_automatic_purchase"},automatic_purchase_limit:{type:"integer",const:0}} },
    ApprovalModel: { type:"object", required:["human_approval_required","agent_self_approval_allowed"], properties:{human_approval_required:{type:"boolean",const:true},agent_self_approval_allowed:{type:"boolean",const:false}} },
    PurchaseEvidenceModel: { type:"object", description:"Required future purchase-attempt evidence fields." },
    MachinePurchasePolicyReadinessResponse: { type:"object", required:["contract_version","machine_purchase_policy_readiness_version","request_id","local_only","machine_purchase_policy_available","automatic_purchase_enabled","payment_execution_enabled","purchase_endpoint_enabled","real_charges_enabled","overall","policy_model","spending_control_model","approval_model","purchase_evidence_model","checks","required_before_automatic_purchase","recommended_machine_purchase_controls","safety_statement"], properties:{contract_version:contractVersion,machine_purchase_policy_readiness_version:{type:"string",const:"atg.machine-purchase-policy-readiness.v1"},request_id:{type:"string"},local_only:{type:"boolean",const:true},machine_purchase_policy_available:{type:"boolean",const:true},automatic_purchase_enabled:{type:"boolean",const:false},payment_execution_enabled:{type:"boolean",const:false},purchase_endpoint_enabled:{type:"boolean",const:false},real_charges_enabled:{type:"boolean",const:false},overall:{type:"object"},policy_model:{$ref:"#/components/schemas/PolicyModel"},spending_control_model:{$ref:"#/components/schemas/SpendingControlModel"},approval_model:{$ref:"#/components/schemas/ApprovalModel"},purchase_evidence_model:{$ref:"#/components/schemas/PurchaseEvidenceModel"},checks:{type:"array",items:{$ref:"#/components/schemas/MachinePurchasePolicyReadinessCheck"}},required_before_automatic_purchase:{type:"array",items:{type:"string"}},recommended_machine_purchase_controls:{type:"array",items:{type:"string"}},safety_statement:{type:"string"}} },
    ...
    PlaceholderBillingPlan: {
      type: "object",
      required: ["plan_code", "label", "price", "billing_interval", "decision_allowance", "rate_limit_max_requests", "payment_required", "automatic_purchase_available"],
      properties: {
        plan_code: { type: "string" }, label: { type: "string" }, price: { type: "null" },
        billing_interval: { type: "string", enum: ["none", "future", "future_usage_based"] },
        decision_allowance: { type: ["integer", "null"], minimum: 0 },
        rate_limit_max_requests: { type: ["integer", "null"], minimum: 0 },
        payment_required: { type: "boolean", const: false },
        automatic_purchase_available: { type: "boolean", const: false },
      },
    },
    PlanModel: {
      type: "object",
      required: ["plan_catalogue_available", "plans_are_placeholders", "currency", "billing_mode", "plans", "source"],
      properties: {
        plan_catalogue_available: { type: "boolean", const: true }, plans_are_placeholders: { type: "boolean", const: true },
        currency: { type: "string", const: "GBP" }, billing_mode: { type: "string", const: "not_enabled" },
        plans: { type: "array", items: { $ref: "#/components/schemas/PlaceholderBillingPlan" } },
        source: { type: "object", description: "Local placeholder-file metadata only." },
      },
    },
    BillingModel: {
      type: "object",
      required: ["concepts", "billing_enabled", "real_invoice_generation_enabled", "receipt_generation_enabled", "tax_calculation_enabled"],
      properties: {
        concepts: { type: "array", items: { type: "string" } }, billing_enabled: { type: "boolean", const: false },
        real_invoice_generation_enabled: { type: "boolean", const: false },
        receipt_generation_enabled: { type: "string", const: "placeholder_only" },
        tax_calculation_enabled: { type: "boolean", const: false },
      },
    },
    PaymentModel: {
      type: "object",
      required: ["payment_processing_enabled", "payment_provider_configured", "provider", "test_mode_available", "live_mode_available", "card_collection_enabled", "bank_payment_enabled", "crypto_payment_enabled", "machine_to_machine_payment_enabled", "automatic_purchase_enabled", "payment_security_ready"],
      properties: {
        payment_processing_enabled: { type: "boolean", const: false }, payment_provider_configured: { type: "boolean", const: false }, provider: { type: "null" },
        test_mode_available: { type: "boolean", const: false }, live_mode_available: { type: "boolean", const: false },
        card_collection_enabled: { type: "boolean", const: false }, bank_payment_enabled: { type: "boolean", const: false }, crypto_payment_enabled: { type: "boolean", const: false },
        machine_to_machine_payment_enabled: { type: "boolean", const: false }, automatic_purchase_enabled: { type: "boolean", const: false }, payment_security_ready: { type: "boolean", const: false },
      },
    },
    MachinePurchaseModel: {
      type: "object",
      required: ["machine_purchase_supported_in_future", "automatic_purchase_enabled", "purchase_authorization_model", "agent_purchase_policy_required", "human_approval_required_before_enabling", "entitlement_upgrade_signal_available", "purchase_endpoint_available", "payment_execution_available"],
      properties: {
        machine_purchase_supported_in_future: { type: "boolean", const: true }, automatic_purchase_enabled: { type: "boolean", const: false },
        purchase_authorization_model: { type: "string", const: "future" }, agent_purchase_policy_required: { type: "boolean", const: true },
        human_approval_required_before_enabling: { type: "boolean", const: true }, entitlement_upgrade_signal_available: { type: "boolean", const: true },
        purchase_endpoint_available: { type: "boolean", const: false }, payment_execution_available: { type: "boolean", const: false },
      },
    },
    BillingPaymentReadinessResponse: {
      type: "object",
      required: ["contract_version", "billing_payment_readiness_version", "generated_at", "request_id", "local_only", "billing_enabled", "payment_processing_enabled", "automatic_purchase_enabled", "real_charges_enabled", "payment_provider_configured", "pci_scope_assessed", "overall", "plan_model", "billing_model", "payment_model", "machine_purchase_model", "checks", "required_before_payments", "recommended_payment_controls", "safety_statement"],
      properties: {
        contract_version: contractVersion, billing_payment_readiness_version: { type: "string", const: "atg.billing-payment-readiness.v1" },
        generated_at: { type: "string", format: "date-time" }, request_id: { type: "string" }, local_only: { type: "boolean", const: true },
        billing_enabled: { type: "boolean", const: false }, payment_processing_enabled: { type: "boolean", const: false }, automatic_purchase_enabled: { type: "boolean", const: false },
        real_charges_enabled: { type: "boolean", const: false }, payment_provider_configured: { type: "boolean", const: false }, pci_scope_assessed: { type: "boolean", const: false },
        overall: { type: "object", required: ["billing_payment_readiness_percent", "status", "next_gate"], properties: {
          billing_payment_readiness_percent: { type: "integer", minimum: 0, maximum: 100 }, status: { type: "string", const: "local_billing_payment_planning_only" },
          next_gate: { type: "string", const: "define_payment_provider_and_billing_controls_before_enabling_payments" },
        } },
        plan_model: { $ref: "#/components/schemas/PlanModel" }, billing_model: { $ref: "#/components/schemas/BillingModel" },
        payment_model: { $ref: "#/components/schemas/PaymentModel" }, machine_purchase_model: { $ref: "#/components/schemas/MachinePurchaseModel" },
        checks: { type: "array", items: { $ref: "#/components/schemas/BillingPaymentReadinessCheck" } },
        required_before_payments: { type: "array", items: { type: "string" } }, recommended_payment_controls: { type: "array", items: { type: "string" } },
        safety_statement: { type: "string", description: "No billing, payment processing, payment-detail collection, automatic purchase, real invoices, public service, or action execution." },
      },
    },
