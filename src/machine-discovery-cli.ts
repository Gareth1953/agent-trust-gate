...
function renderMachineDiscovery(record: MachineDiscoveryRecord, summaryOnly: boolean): string {
  ...
  const lines = [
    ...
    `- application: ${record.embeddedCommerceGatepass.commercialApplication}`,
    `- command: ${record.embeddedCommerceGatepass.command}`,
    `- target audiences: ${record.embeddedCommerceGatepass.targetAudiences.join(", ")}`,
    `- evaluation scope: ${record.embeddedCommerceGatepass.evaluationScope.join(", ")}`,
    `- commercial route: ${record.embeddedCommerceGatepass.commercialRoute}`,
