...
function renderMachineDiscovery(record: MachineDiscoveryRecord, summaryOnly: boolean): string {
  ...
  const lines = [
    ...
    `expected Pages URL: ${MACHINE_DISCOVERY_EXPECTED_PAGES_URL}`,
    "",
    ...
    `- GitHub Pages activation pending: ${record.statuses.githubPagesActivationPending}`,
    `- GitHub Pages live verification pending: ${record.statuses.githubPagesLiveVerificationPending}`,
