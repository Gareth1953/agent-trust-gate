...
export interface CustomerTenantReadinessReport {
  ...
  overall: {
    customer_tenant_readiness_percent: 32;
...
function readinessChecks(config: LocalCustomerTenantsResult): CustomerTenantReadinessCheck[] {
  return [
    ...
    check("machine_purchase_policy_readiness_available", "Machine purchase policy readiness available", "partial", "warning", ["Future purchase ownership requires tenant, account, plan, entitlement, approval, and evidence fields."], "Implement production ownership and authorization before any purchase path."),
